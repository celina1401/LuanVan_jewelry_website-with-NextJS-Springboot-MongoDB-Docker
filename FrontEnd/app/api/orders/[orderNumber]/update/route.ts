// app/api/orders/[orderNumber]/update/route.ts
...
export async function PUT(
  ...
  try {
    ...
    console.log('Update order request:', { orderNumber, body });
    ...
    let url = `${ORDER_SERVICE_URL}/api/orders/${orderNumber}/${action}`;
    ...
    try {
      // Helper to fetch order details by orderNumber
      const fetchOrderDetails = async (): Promise<any | null> => {
        try {
          const res = await fetch(`${ORDER_SERVICE_URL}/api/orders/number/${orderNumber}`, { headers });
          if (res.ok) {
        ...
        if (isCompleted && order.userId && (order.total ?? order.totalAmount ?? order.amount)) {
          ...
          try {
            const membershipRes = await fetch(`${USER_SERVICE_URL}/api/users/membership/${order.userId}/purchase`, {
              ...
              body: JSON.stringify({ orderAmount, orderId: order.id ?? orderNumber, orderNumber: order.orderNumber ?? orderNumber }),
            });
